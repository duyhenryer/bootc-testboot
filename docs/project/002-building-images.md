...
**Tier 2 env files** are generated by `testboot-app-setup.service` at first boot. It reads passwords generated by infra setup services (e.g., `/var/lib/mongodb/.admin-pw`) and writes standardized env files with variables like `MONGODB_URI`, `REDIS_URL`, `AMQP_URL`. Each app declares only the infra files it actually uses.
