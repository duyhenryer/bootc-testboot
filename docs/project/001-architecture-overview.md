...
| `make verify-ghcr` | After CI published to GHCR | Runs [`scripts/verify-ghcr-packages.sh`](../../scripts/verify-ghcr-packages.sh): `skopeo inspect` and `podman pull` of **remote** tags, validates artifact paths. Run on a dev machine with free disk; see [009-ghcr-audit.md](009-ghcr-audit.md). Use `VERIFY_SKIP_PULL=1` for metadata-only. |
...
Configs in `/usr/share/` are symlinked from `/etc/` at build time (`ln -sf`), making them read-only at runtime while services still find them at the expected `/etc/` path. For the full mapping of every file, see [008-rootfs-overlay-guide.md](008-rootfs-overlay-guide.md).
